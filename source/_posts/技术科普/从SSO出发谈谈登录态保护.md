...
1. 完成在 SSO 登录中心的登录后，登录中心创建一个全局会话
1. SSO 登录中心返回一个 tikect 给 A 站点，并在 SSO 登录中心记录下 A 站点
1. 下次访问 A 站点时携带包含了这个 ticket 的 Cookie，A 站点收到请求并创建针对 A 站点的局部会话，给用户返回已登录的 A 站点页面

此时如果用户想要访问 B 站点，那么流程如下所示：

1. 浏览器访问 B 站点显示未登录，跳转至 SSO 登录中心
1. SSO 登录中心发现用户已经在登录中心完成登录
1. SSO 登录中心返回一个 tikect 给 B 站点
1. B 站点拿到 ticket 后再请求一次 SSO 站点，验证无误后写入 ticket 到 Cookie 中，此时 SSO 登录中心记录下 B 站点
1. 下次访问 B 站点时携带包含了这个 ticket 的 Cookie，B 站点收到请求并创建针对 B 站点的局部会话，给用户返回已登录的 B 站点页面
...
1. 后端方向，通过某种途径，可以**让 B 站点的后端解析来自 A 站点中包含的已经登录过 SSO 的 Cookie**。
